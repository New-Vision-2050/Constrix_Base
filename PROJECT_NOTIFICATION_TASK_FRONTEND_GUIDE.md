...
> **Version**: 1.3 (aligned with backend lifecycle refinement; added mobile inbox/confirm-receive, `assigned_user` action taker, and `InsideCustomLocations`-only conditions)  
> **Scope**: Frontend integration guide for the **الصيانة والطوارئ** (Maintenance & Emergencies) project-detail tab and its first sub-view **الإشعارات** (Notifications), including the **Add Notification** 5-step wizard and the mobile confirm-receive flow.  
> **Backend API reference**: `PROJECT_NOTIFICATION_TASK_IMPLEMENTATION_PLAN.md` (v1.3) and `PROJECT_MODULE_DEEP_DOCUMENTATION.md`  
> **Language**: Arabic UI (RTL) with English translation keys.

> ### Recent changes in this version
> - `POST /projects/notifications/{id}/start` is replaced by `POST /projects/notifications/{id}/confirm-receive` (form `confirmProjectNotificationPresence`). The old `/start` endpoint remains as a legacy alias.
> - Removed `startProjectNotificationTask` from the internal form list.
> - New procedure step action taker type: `assigned_user` — resolves to the engineer selected in the wizard.
> - `createProjectNotificationTask` conditions are now limited to `InsideCustomLocations` only. The frontend should not block submission on shift/GPS/holiday checks.
...
| `/api/v1/procedure-settings/approval-responsibles?type=project_notification_task&form=createProjectNotificationTask` | GET | Preview approvers / auto-approve status for the project-notification parent. |
| `/api/v1/projects/notifications/my-tasks` | GET | Mobile: list notifications assigned to the current employee after confirm-receive (with filters). |
| `/api/v1/projects/notifications/my-inbox` | GET | Mobile: approved notifications waiting for confirm-receive (with filters). |
| `/api/v1/projects/notifications/my-inbox-counts` | GET | Mobile: status counts for the employee's notifications. |
| `/api/v1/projects/notifications/filters` | GET | Mobile: filter metadata (statuses, projects, duration). |
| `/api/v1/projects/notifications/{id}/available-actions` | GET | Mobile: list currently available internal procedure actions (e.g. تأكيد استلام, تحديث, إنهاء). |
| `/api/v1/projects/notifications/{id}/confirm-receive` | POST | Mobile: confirm-receive / تأكيد استلام. Sends `latitude`, `longitude`, `internal_procedure_setting_id`, `notes`. Replaces the old `/start` endpoint. |
| `/api/v1/projects/notifications/{id}/start` | POST | Legacy alias for confirm-receive. Prefer `/confirm-receive`. |
| `/api/v1/projects/notifications/{id}/take-action` | POST | Mobile: record any available internal procedure action (Update). Sends `internal_procedure_setting_id`, optional `latitude`, `longitude`, `notes`. |
...
```
GET /api/v1/procedure-settings/approval-responsibles?type=project_notification_task&form=createProjectNotificationTask
```
...
### 10A.5 Procedure Step Configuration & Conditions (Frontend Impact)

Project-notification procedures live under the dedicated parent **مهام الصيانة والطوارئ** of type `project_notification_task`. The internal forms are:

| Form (Arabic) | Form key | Purpose |
|---------------|----------|---------|
| إنشاء إشعار مشروع | `createProjectNotificationTask` | Creation approval/assignment (dashboard). |
| تأكيد استلام | `confirmProjectNotificationPresence` | Mobile confirm-receive (replaces the removed `startProjectNotificationTask`). |
| تحديث | `updateProjectNotificationTask` | Optional mid-task update. |
| إنهاء المهمة | `endProjectNotificationTask` | Task completion. |

#### Action Taker Type: `assigned_user`

When admins configure procedure steps in **الإعدادات > الإجراءات الداخلية**, a new action-taker type is available: **`assigned_user`** (الموظف المُسند). Use it for the **create** step when the notification should be sent directly to the engineer selected in the wizard without requiring a separate approver.

- Backend resolves `assigned_user` to the `EmployeeTaskRequest.user_id` (the engineer selected in step 4 of the wizard).
- If the create step uses `assigned_user` and the workflow is auto-approved, the notification lands in the engineer's mobile inbox (`GET /my-inbox`) immediately after creation.
- Only the `approve` form is permitted for `assigned_user` steps (other forms are rejected by validation).

Example step payload admins send to `POST /procedure-settings/{id}/steps`:

```json
{
  "name": "إرسال للمهندس المُسند",
  "step_order": 1,
  "is_approve": true,
  "action_taker_type": "assigned_user",
  "action_taker_user_ids": [],
  "action_taker_management_hierarchies": [],
  "action_taker_procedure_settings": []
}
```

#### Conditions: Only `InsideCustomLocations` for creation

For `createProjectNotificationTask`, the backend now enforces **only** the `InsideCustomLocations` condition. The admin's real-time context (shift, current GPS, holiday) is irrelevant because the admin creates the notification on behalf of the engineer.

**What the frontend must do:**

1. **Do not block submission** based on employee shift, current GPS, or holiday — the backend will not evaluate those conditions for project notifications.
2. **Do respect `InsideCustomLocations`**: If the admin enabled this condition and configured custom polygonal areas, the backend validates the wizard's `task_latitude`/`task_longitude` against those polygons. If the location is outside, creation returns **HTTP 422** with the condition error in `message.validations` or `message.description`.
3. **Show the condition state**: When calling `GET /procedure-settings/forms-conditions?form=createProjectNotificationTask`, the response contains only `InsideCustomLocations`. Render only that condition card in the condition editor UI; do not show disabled/grayed-out cards for the other conditions.
4. **Map drawing**: If `InsideCustomLocations` is active, consider visually indicating the allowed polygons on the location map (optional UX enhancement; the backend already validates the pin).

For the confirm-receive form (`confirmProjectNotificationPresence`), the backend checks only `AllowOnHolidays`. The mobile app should handle a holiday-block error the same way it handles regular employee-task start errors.
...
- `GET /projects/notifications/my-tasks` — same filters as the dashboard list, but restricted to the current employee's `assigned_user_id` (after confirm-receive).
- `GET /projects/notifications/my-inbox` — approved notifications waiting for the employee to confirm-receive (same filters as my-tasks). Status is always `approved`.
- `GET /projects/notifications/my-inbox-counts` — status counts for the employee's notifications (badge).
- `GET /projects/notifications/filters` — filter metadata (statuses, projects, duration) for the mobile filter UI.
...
- `POST /projects/notifications/{id}/confirm-receive` — records `confirmProjectNotificationPresence` (تأكيد استلام) and starts the task session. This is the new confirm-receive step; the old `startProjectNotificationTask` form has been removed.
- `POST /projects/notifications/{id}/take-action` — records any available action such as `updateProjectNotificationTask` (تحديث). Payload includes `internal_procedure_setting_id` and optional `latitude`/`longitude`.
...
1. The engineer opens the **inbox** (`GET /my-inbox`). Every item has `status = approved` and is waiting for confirm-receive.
2. The engineer taps **تأكيد استلام** → call `POST /{id}/confirm-receive` with the `internal_procedure_setting_id` returned by `available-actions` for the form `confirmProjectNotificationPresence`.
3. On success the task moves to `in_progress` and disappears from the inbox; it now appears in **my tasks** (`GET /my-tasks`).
4. Later the engineer calls `POST /{id}/end` (form `endProjectNotificationTask`) to complete it.
...
- [ ] Mobile inbox (`GET /my-inbox`) only shows `approved` notifications.
- [ ] Mobile confirm-receive (`POST /confirm-receive`) uses form `confirmProjectNotificationPresence` and moves the task to `in_progress`.
- [ ] After confirm-receive, the task appears in `GET /my-tasks`, not in `GET /my-inbox`.
- [ ] Mobile inbox counts and filter metadata populate the badge/filter UI.
- [ ] Procedure step with `action_taker_type = assigned_user` sends the notification to the selected engineer without a separate approver.
- [ ] Wizard creation is not blocked by shift/GPS/holiday checks on the frontend; only `InsideCustomLocations` (if enabled) can block via backend 422.
