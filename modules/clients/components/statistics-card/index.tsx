...
import { useClientsDataCxt } from "../../context/ClientsDataCxt";
...
export default function ClientsStatisticsCards() {
  ...
  const { widgetsData, widgetsLoading } = useClientsDataCxt();
  // from context
  ...
  const cardsData = [
    {
      ...
      value: Number(widgetsData?.[0]?.total || "0").toFixed(2),
      percentage: Number(widgetsData?.[0]?.percentage || "0").toFixed(2),
      percentageColor: "#27C200",
      icon: <UserIcon size={24} color="#B39DDB" />,
    },
    {
      label: t("totalClientsInLastMonth"),
      value: Number(widgetsData?.[1]?.total || "0").toFixed(2),
      percentage: Number(widgetsData?.[1]?.percentage || "0").toFixed(2),
      percentageColor: "#FF2D2D",
      icon: <UsersIcon size={24} color="#B39DDB" />,
    },
    {
      label: t("activeClients"),
      value: Number(widgetsData?.[2]?.total || "0").toFixed(2),
      percentage: Number(widgetsData?.[2]?.percentage || "0").toFixed(2),
      percentageColor: "#27C200",
      icon: <ActivityIcon size={24} color="#B39DDB" />,
    },
    {
      label: t("inactiveClients"),
      value: Number(widgetsData?.[3]?.total || "0").toFixed(2),
      percentage: Number(widgetsData?.[3]?.percentage || "0").toFixed(2),
