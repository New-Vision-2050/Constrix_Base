...
import ProjectAttachmentsGridItem from "./ProjectAttachmentsGridItem";
...
export default function ProjectAttachmentsGridView() {
  const {
    docs,
    docsPagination,
    isLoadingDocs,
      ...
      <div className="mt-8 flex justify-center w-full">
        <div className={isLoadingDocs ? "opacity-50 pointer-events-none" : ""}>
          <Pagination
            ...
            onPageChange={(page) => tableParams.setPage(page)}
            currentLimit={docsPagination?.per_page ?? 10}
            limitOptions={[10, 25, 50, 100]}
            onLimitChange={(limit) => tableParams.setLimit(limit)}
          />
        </div>
      </div>
    </div>
