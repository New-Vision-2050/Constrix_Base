...
import WaterProjectNotificationsView from "./components/ProjectNotificationsView";

const WaterSiteStatusTypesTab = dynamic(
  () => import("./components/site-status-types"),
  { ssr: false },
);

const WaterProjectNotificationChartsView = dynamic(
...
export default function MaintenanceEmergencyWaterTab() {
  const t = useTranslations("project.maintenanceEmergencyWater");
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box key={activeTab}>
        {activeTab === "notifications" && <WaterProjectNotificationsView />}
        {activeTab === "siteStatusTypes" && (
          <WaterSiteStatusTypesTab projectTypeId={project?.projectData?.sub_sub_project_type_id} />
        )}
        {activeTab === "indicators" && <WaterProjectNotificationChartsView />}
