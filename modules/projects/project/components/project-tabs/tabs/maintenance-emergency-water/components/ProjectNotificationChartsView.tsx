...
import { useProjectWaterNotificationCharts } from "@/modules/projects/project/query/useProjectWaterNotificationCharts";
import type { WaterChartFilters } from "@/modules/projects/project/query/useProjectWaterNotificationCharts";
...
function DimensionBarChart({
  ...
  filterKey?: keyof WaterChartFilters;
  onToggle: (key: keyof WaterChartFilters, code: string, label?: string) => void;
  total: number;
}) {
  const theme = useTheme();
...
function ChartsContent({
  ...
  isFetching: boolean;
  filters: WaterChartFilters;
  filterLabels?: Partial<Record<keyof WaterChartFilters, string>>;
  statusKpis: any[];
  t: (key: string) => string;
  tCharts: (key: string) => string;
  theme: any;
  onToggleFilter: (key: keyof WaterChartFilters, code: string, label?: string) => void;
  onClearFilters: () => void;
  onRefetch: () => void;
  searchInput: string;
      ...
      {activeFilterCount > 0 && (
        <Stack direction="row" spacing={0.5} flexWrap="wrap" sx={{ mb: 2, gap: 0.5 }}>
          {Object.entries(filters).map(([key, value]) => {
            ...
            const displayKey = getFilterKeyLabel(key as keyof WaterChartFilters, t, tCharts);
            const displayValue = filterLabels?.[key as keyof WaterChartFilters] ?? value;
            return (
              <Chip
                key={key}
                label={`${displayKey}: ${displayValue}`}
                size="small"
                onDelete={() => onToggleFilter(key as keyof WaterChartFilters, value as string)}
                deleteIcon={<X />}
                sx={{
                  borderRadius: 2,
                  bgcolor: alpha(theme.palette.primary.main, 0.08),
...
export default function ProjectNotificationChartsView() {
  ...
  const [filters, setFilters] = useState<WaterChartFilters>({});
  const [filterLabels, setFilterLabels] = useState<Partial<Record<keyof WaterChartFilters, string>>>({});
  ...
  const { data, isLoading, isFetching, refetch } = useProjectWaterNotificationCharts(scope, filters);

  const handleToggleFilter = useCallback(
    (key: keyof WaterChartFilters, code: string, label?: string) => {
      setFilters((prev) => {
