...
import {
  useWaterSiteStatusTypeKeys,
  useCreateWaterSiteStatusTypeKeyMutation,
  useUpdateWaterSiteStatusTypeKeyMutation,
  useDeleteWaterSiteStatusTypeKeyMutation,
} from "@/modules/projects/project/query/useWaterSiteStatusTypes";
import type {
  SiteStatusTypeKey,
  SiteStatusTypeWithKeys,
} from "@/services/api/projects/notifications-water/types/response";
import type { UseWaterSiteStatusTypesParams } from "@/modules/projects/project/query/useWaterSiteStatusTypes";
...
export default function SiteStatusTypeKeysDrawer({
  ...
  const t = useTranslations("project.maintenanceEmergencyWater.siteStatusTypes");
  const typeId = siteStatusType?.id;

  const { data, isLoading } = useWaterSiteStatusTypeKeys(typeId);
  const keys = data ?? [];
  ...
  const createKeyMutation = useCreateWaterSiteStatusTypeKeyMutation(typeId, listParams);
  const updateKeyMutation = useUpdateWaterSiteStatusTypeKeyMutation(typeId, listParams);
  const deleteKeyMutation = useDeleteWaterSiteStatusTypeKeyMutation(typeId, listParams);
...
interface SiteStatusTypeKeyFormDialogProps {
  ...
  createMutation: ReturnType<typeof useCreateWaterSiteStatusTypeKeyMutation>;
  updateMutation: ReturnType<typeof useUpdateWaterSiteStatusTypeKeyMutation>;
}
