...
import {
  ...
  useProjectWaterNotificationAvailableActions,
...
import type { ProjectNotificationAttachment } from "@/services/api/projects/notifications-water/types/response";
...
export default function NotificationDetailView({
  ...
  const { data: notification, isLoading, isError } = useProjectWaterNotificationDetail(
    notificationScope,
    notificationId,
  );
  ...
  const { data: availableActions = [] } = useProjectWaterNotificationAvailableActions(
    notificationId,
  );
