...
export default function SchemaLevelTabs({ parentId }: SchemaLevelTabsProps) {
  ...
  useEffect(() => {
    if (thirdLevelQuery.data && thirdLevelQuery.data.length > 0) {
      setSelectedSchema(thirdLevelQuery.data[0]);
    }
  }, [thirdLevelQuery.data]);
        ...
        <Grid size={9}>
          {selectedSchema && !isLoading && selectedTab && data && (
            <div className="space-y-4">
              ...
              <Paper className="p-4">
                {(() => {
                  const activeTab = filteredTabs.find((tab) => tab.value === selectedTab);
