...
function ProjectTermsView({
  ...
  const [selectedItem, setSelectedItem] = useState<PRJ_ProjectTerm | null>(
    null,
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row: PRJ_ProjectTerm) => (
        <Typography variant="body2">{row.sub_items_count}</Typography>
      ),
    },
    {
      key: "services",
    ...
    },
    {
      ...
      render: (row: PRJ_ProjectTerm) => (
        <Box display="flex" alignItems="center" gap={1}>
          <Switch checked={row.status === "1"} size="small" color="primary" />
  ...
  const handleDelete = () => {
    if (selectedItem) {
      const index = mockProjectItems.findIndex(
