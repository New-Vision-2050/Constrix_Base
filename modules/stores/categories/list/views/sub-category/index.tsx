...
import { getSubCategoryColumns, CategoryRow } from "./columns";

const SUB_CATEGORIES_QUERY_KEY = "sub-categories-list";
const SubCategoriesTableLayout = HeadlessTableLayout<CategoryRow>("ssct");
...
function SubCategoriesView() {
  ...
  const params = SubCategoriesTableLayout.useTableParams({
    initialPage: 1,
    initialLimit: 10,
    initialSortBy: "name",
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(
        `${baseURL}/ecommerce/dashboard/categories?depth=1&page=${params.page}&per_page=${params.limit}&search=${params.search || ""}&sort_by=${params.sortBy || ""}&sort_direction=${params.sortDirection}`,
      );
  ...
  const state = SubCategoriesTableLayout.useTableState({
    data: rows,
    columns,
    totalPages,
    totalItems,
  ...
  return (
    <Box py={2}>
      ...
      <SubCategoriesTableLayout
        filters={
          <SubCategoriesTableLayout.TopActions
            state={state}
            customActions={
        ...
        table={
          <SubCategoriesTableLayout.Table
            state={state}
            loadingOptions={{ rows: 5 }}
          />
        }
        pagination={<SubCategoriesTableLayout.Pagination state={state} />}
      />
