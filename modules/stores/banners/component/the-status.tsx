  ...
  const handleConfirm = async () => {
    try {
      const response = await apiClient.patch(
        `/ecommerce/dashboard/banners/${id}/toggle-active`
      );
      ...
      if (response.status === 200) {
        ...
        setShowDialog(false);
        toast({
          title: t("banner.success") || "Success",
          description: tempIsActive
            ? t("banner.activatedSuccessfully") ||
              "Banner activated successfully"
            : t("banner.deactivatedSuccessfully") ||
              "Banner deactivated successfully",
        });
      } else {
        toast({
          title: t("banner.error") || "Error",
          description:
            t("banner.failedToUpdateStatus") || "Failed to update status",
          variant: "destructive",
        });
      }
    } catch {
    ...
    <>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <ConfirmationDialog
          ...
          description={
            !isActive
              ? t("banner.areYouSureReactivate") ||
                "Are you sure you want to reactivate this banner?"
              : t("banner.areYouSureDeactivate") ||
