...
export function ContactDialog({ open, onClose, onSuccess, pageId }: ContactDialogProps) {
  ...
  const { data: pageData, isLoading: isFetching } = useQuery({
    queryKey: ["page-contact", pageId],
    queryFn: () => apiClient.get(`/ecommerce/dashboard/pages/contact/${pageId}`),
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm<PageFormData>({
    ...
    defaultValues: { name: "", type: "التواصل" },
  });
  ...
  useEffect(() => {
    if (isEditMode && pageData?.data?.payload && open) {
      ...
      reset({ name: page.name || "", type: page.type || "التواصل" });
  ...
  const onSubmit = async (data: PageFormData) => {
    try {
      ...
      if (isEditMode && pageId) {
        await apiClient.put(`/ecommerce/dashboard/pages/contact/${pageId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        await apiClient.post("/ecommerce/dashboard/pages/contact", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <Select value={watch("type")} onValueChange={(value) => setValue("type", value)} disabled={isSubmitting || isFetching}>
              ...
              <SelectContent>
                <SelectItem value="التواصل">التواصل</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
