...
export default function FeaturedDealDialog({
  ...
  const onSubmit = async (data: FeaturedDealFormData) => {
    try {
      // API call here
      toast.success(
        isEditMode ? "تم تحديث الصفقة المميزة بنجاح" : "تم إضافة الصفقة المميزة بنجاح"
      );
      onSuccess?.();
      reset();
      onClose();
    } catch (error: any) {
      toast.error("فشل في حفظ الصفقة المميزة");
    }
  };
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-center text-lg font-semibold text-white">
            {isEditMode ? "تعديل صفقة مميزة" : "إضافة صفقة مميزة"}
          </DialogTitle>
        </DialogHeader>
