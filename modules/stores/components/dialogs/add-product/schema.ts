...
// Helper function to create schema based on edit mode
export const createProductSchemaForMode = (isEditMode = false) =>
  z.object({
    name: z.string().min(1, t("product.validation.name.required")),
    description: z
      .string()
      .min(1, t("product.validation.description.required")),
    price: z
      .string()
      .refine(
    ...
    // Files - make main_image optional in edit mode
...
// Main product schema (for backward compatibility)
export const createProductSchema = createProductSchemaForMode(false);

export type CreateProductFormData = z.infer<
  ReturnType<typeof createProductSchemaForMode>
>;
