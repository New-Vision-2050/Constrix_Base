...
const LegalDataSection = ({
  ...
  const permissions = can([PERMISSION_ACTIONS.VIEW, PERMISSION_ACTIONS.UPDATE , PERMISSION_ACTIONS.CREATE] , PERMISSION_SUBJECTS.COMPANY_PROFILE_LEGAL_DATA) as {
    VIEW: boolean;
    UPDATE: boolean;
    CREATE: boolean;
  }

    const { data, isPending, isSuccess ,...rest } = useQuery({
