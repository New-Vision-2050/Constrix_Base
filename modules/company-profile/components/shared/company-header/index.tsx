...
const CompanyHeader = () => {
  ...
  const logo = data?.payload?.logo;
  const companyName = data?.payload?.name ?? "";
  ...
  return (
    <div className="bg-sidebar rounded-lg w-full flex items-center justify-between p-4">
      <CompanyLogo logo={logo} isPending={isPending} />

      {isPending && (
        <div className="flex flex-col text-right w-full pr-6 gap-y-3">
