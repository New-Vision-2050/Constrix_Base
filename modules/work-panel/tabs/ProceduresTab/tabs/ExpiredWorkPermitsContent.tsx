...
const QUERY_KEY = "expired-work-permits";

// Create typed table instance
const ExpiredWorkPermitsTable =
  HeadlessTableLayout<InfoAlertItem>("expired-work-permits");

// Column definitions
const getExpiredWorkPermitsColumns = (t: (key: string) => string) => [
  {
    key: "name",
    name: t("name"),
    sortable: false,
  ...
  },
  {
    ...
    name: t("workPermit"),
    sortable: false,
    render: (row: InfoAlertItem) => <span>{row.work_permit}</span>,
  },
  {
    key: "end_date",
    name: t("endDate"),
...
export default function ExpiredWorkPermitsContent() {
  ...
  // Table params
  const params = ExpiredWorkPermitsTable.useTableParams({
    initialPage: 1,
    initialLimit: 10,
  });
  ...
  const columns = [
    ...getExpiredWorkPermitsColumns(t),
    {
      key: "actions",
      name: t("actions"),
      sortable: false,
  ...
  // Table state
  const state = ExpiredWorkPermitsTable.useTableState({
    data,
    columns,
    totalPages,
    totalItems,
  ...
  return (
    <Box sx={{ p: 3 }}>
      <ExpiredWorkPermitsTable
        filters={
          <ExpiredWorkPermitsTable.TopActions state={state}>
          </ExpiredWorkPermitsTable.TopActions>
        }
        table={
          <ExpiredWorkPermitsTable.Table
            state={state}
            loadingOptions={{ rows: 5 }}
          />
        }
        pagination={<ExpiredWorkPermitsTable.Pagination state={state} />}
      />
    </Box>
  );
}
