    ...
    columns: [
      {
        ...
        render: (_: unknown, row: BouquetTableRow) => (
          <div
            className={
              can(PERMISSIONS.package.view)
                ? "flex gap-3 border-e-2 cursor-pointer"
                : "flex gap-3 border-e-2"
            }
            onClick={() =>
              can(PERMISSIONS.package.view) &&
              router.push(`/bouquetDetails/${row.id}`)
            }
          >
            <div>
        ...
        render: (_: unknown, row: BouquetTableRow) => {
          return (
            <p className="font-medium">
              {row.subscription_period} {t(`${row.subscription_period_unit}`)}
            </p>
          );
        },
      },
