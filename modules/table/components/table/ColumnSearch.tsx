...
const ColumnSearch: React.FC<ColumnSearchProps> = ({
  ...
  const buildDependenciesMap = useCallback(() => {
    ...
    searchableColumns.forEach(column => {
      if (column.searchType?.type === 'dropdown' && column.searchType.dynamicDropdown?.dependsOn) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
      {searchableColumns.map(column => {
        ...
        const displayValue = localInputValues[column.key] !== undefined 
          ...
          case 'dropdown':
            return (
              <DropdownSearch
                ...
                placeholder={searchType.placeholder || `Filter by ${column.label.toLowerCase()}`}
              />
            );
