 ...
 */
const AttendanceHistoryItem: React.FC<AttendanceHistoryItemProps> = ({
  record,
}) => {
  const t = useTranslations("AttendanceDepartureModule.Map.employeeDetails");
  const sheetT = useTranslations("AttendanceDepartureModule.EmployeeDetailsSheet");
  ...
  const getStatusText = () => {
    ...
    if (record.is_holiday) return t("holiday");
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <div className={`text-xs px-2 py-1 rounded-full ${getStatusColor()}`}>
          {sheetT("status.attendanceStatus") + " : " + getStatusText()}
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          <div className="text-xs text-[#FF3B8B] flex items-center gap-1 mb-1">
