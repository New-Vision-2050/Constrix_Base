...
export default function ProfileRoleSelector({
  id,
  userTypes,
  readonly,
}: PropsT) {
  ...
  const role = isUserProfile
    ? UsersRole.Employee
    : searchParams.get("role") ?? "2";
  // current role
  const defaultRole = useMemo(() => {
    return readonly || isUserProfile
      ? UsersRole.Employee
      : role == "2"
      ? UsersRole.Client
      : UsersRole.Broker;
  ...
  return (
    <>
      <Button
        ...
        endIcon={<MoreVert />}
      >
        {profileRole == UsersRole?.Client
          ? t("header.userTypes.client")
          : profileRole == UsersRole?.Broker
          ? t("header.userTypes.broker")
          : t("header.userTypes.employee")}
      </Button>
      <Menu
        id="user-menu"
        anchorEl={anchorEl}
        ...
        {!readonly &&
          ...
          userTypes.map((userType) => (
            <MenuItem
              component={Link}
              href={
                userType.role == UsersRole.Employee
                  ? `/user-profile?id=${id}&role=${UsersTypes.Employee}`
                  : `/client-profile/${id}?role=${userType.role}`
              }
              key={userType.id}
            ...
            >
              ...
              <ListItemText>
                {userType.role == UsersRole.Client
                  ? t("header.userTypes.client")
                  : userType.role == UsersRole.Broker
                  ? t("header.userTypes.broker")
                  : t("header.userTypes.employee")}
              </ListItemText>
            </MenuItem>
