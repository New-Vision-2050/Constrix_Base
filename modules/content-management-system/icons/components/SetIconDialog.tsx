...
export default function SetIconDialog({
  ...
  const { data: categoriesData } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(
        baseURL + "/website-icons/category-types"
      );
  ...
  const onSubmit = async (data: IconFormData) => {
    try {
      ...
      if (isEditMode && iconId) {
        ...
        refetch();
        toast.success(t("updateSuccess") || "Icon updated successfully!");
      } else {
        if (!data.logo_image) {
        ...
        toast.success(t("createSuccess") || "Icon created successfully!");
      }
      ...
      toast.error(
        isEditMode
          ? t("updateError") || "Failed to update icon. Please try again."
          : t("createError") || "Failed to create icon. Please try again."
      );
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className={`max-w-4xl w-full bg-sidebar border-gray-700 p-4 sm:p-6 ${
          isRtl ? "rtl" : "ltr"
        }`}
        dir={isRtl ? "rtl" : "ltr"}
      >
                  ...
                  <FormControl>
                    ...
                    >
                      ...
                      >
                        <SelectValue
                          placeholder={
                            t("categoryPlaceholder") || "أختار الفئة"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
