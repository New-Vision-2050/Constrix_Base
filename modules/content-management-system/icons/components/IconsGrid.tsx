...
export default function IconsGrid({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {icons.map((icon) => {
        const isDeleting = deletingIconId === icon.id;
        const filteredActions = iconActions
          ?.filter((action) => action.show)
          .map((action) => ({
            ...action,
            disabled: isDeleting || action.disabled,
          }));

        return (
          <div
            key={icon.id}
            className={`relative ${isDeleting ? "opacity-50" : ""}`}
          >
            <ProjectCard
              id={icon.id}
              title={icon.name_ar}
