...
export const useFormStore = create<FormState>((set) => ({
  ...
  setTouched: (field, isTouched) => set((state) => {
    console.log(`Setting touched for ${field}:`, isTouched);
    return {
  ...
  setAllTouched: () => set((state) => {
    console.log('Setting all fields as touched');
    const allTouched = Object.keys(state.values).reduce((acc, key) => {
      acc[key] = true;
      return acc;
    }, {} as Record<string, boolean>);
    
    console.log('All touched fields:', allTouched);
...
export const validateField = (
  ...
  console.log('Validating with rules:', rules);
    ...
    switch (rule.type) {
      ...
      case 'min':
        if (typeof value === 'number' && value < rule.value) {
          console.log('Min validation failed');
          return rule.message;
        }
        break;
      case 'max':
        if (typeof value === 'number' && value > rule.value) {
          console.log('Max validation failed');
          return rule.message;
        }
        break;
      case 'minLength':
        if (typeof value === 'string' && value.length < rule.value) {
          console.log('MinLength validation failed');
          return rule.message;
        }
        break;
      case 'maxLength':
        if (typeof value === 'string' && value.length > rule.value) {
          console.log('MaxLength validation failed');
          return rule.message;
        }
        break;
      case 'pattern':
