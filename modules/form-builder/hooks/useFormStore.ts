...
const debouncedValidations = new Map<
  string,
...
export const useFormStore = create<FormState>((set, get) => ({
  ...
  setTouched: (formId: string, field: string, isTouched: boolean) =>
    set((state: FormState) => {
      const formState = state.forms[formId] || getDefaultFormState();
  ...
  resetForm: (formId: string, values = {}) =>
    set((state: FormState) => ({
      forms: {
        ...state.forms,
  ...
  setIsValid: (formId: string, isValid: boolean) =>
    set((state: FormState) => {
      const formState = state.forms[formId] || getDefaultFormState();
  ...
  setFieldValidating: (formId: string, field: string, isValidating: boolean) =>
    set((state: FormState) => {
      const formState = state.forms[formId] || getDefaultFormState();
            ...
            if (!isValid) {
              store.setError(
                ...
                response.data?.message ??
                  response.data?.payload?.sentence ??
                  rule.message
              );
            } else {
  ...
  setEditMode: (formId: string, isEditMode: boolean) =>
    set((state: FormState) => {
      const formState = state.forms[formId] || getDefaultFormState();
...
export const useFormInstance = (
  ...
  const setError = useCallback(
    (field: string, error: string | React.ReactNode | null) => {
  ...
  const setFieldValidating = useCallback(
    (field: string, isValidating: boolean) => {
      useFormStore.getState().setFieldValidating(formId, field, isValidating);
    },
    [formId]
  );

  const validateFieldWithApi = useCallback(
    (field: string, value: any, rule: ValidationRule) => {
