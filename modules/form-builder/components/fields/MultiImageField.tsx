...
const MultiImageField: React.FC<MultiImageFieldProps> = ({
  ...
  const [filesInfo, setFilesInfo] = useState<{ name: string; date: string }[]>(
    []
  );
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      files.forEach((file) => {
        ...
        if (
          ...
          const maxSizeMB =
            Math.round((field.imageConfig.maxFileSize / (1024 * 1024)) * 10) /
            10;
          invalidFiles.push({
            file,
      ...
      try {
        ...
        if (uploadedUrls.length < files.length) {
          formInstance.setError(
            field.name,
            `${files.length - uploadedUrls.length} ${t("UploadsFailed")}`
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const newFilesInfo: { name: string; date: string }[] = [];

    normalizedValue.forEach((item) => {
      if (typeof item === "string" || (item as FileWithUrl).url) {
        // For remote images, you might not have name/date, so you can skip or put placeholder
        newFilesInfo.push({
          name: typeof item === "string" ? item : (item as FileWithUrl).url,
          date: "Unknown",
        });
      } else if (item instanceof File) {
        newFilesInfo.push({
    ...
    <div className="relative">
      ...
      >
        ...
        {filesInfo.map((file, index) => (
          <div
            key={index}
            className="flex flex-row items-center justify-between border rounded-md p-2 mb-2"
          >
