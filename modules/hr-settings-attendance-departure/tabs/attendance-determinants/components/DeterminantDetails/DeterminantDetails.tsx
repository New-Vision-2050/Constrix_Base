...
const DeterminantDetails: React.FC<DeterminantDetailsProps> = ({
  ...
  const branchTagHover = isDarkMode ? 'hover:bg-[#2A2045]/80' : 'hover:bg-gray-300';
  ...
  return (
    <div className={`${cardBg} border ${cardBorder} ${cardText} rounded-xl p-2 relative overflow-hidden shadow-sm`}>
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        ...
        <div className="relative">
          ...
          {showDropdown && (
            <div className={`absolute left-0 mt-2 w-32 ${dropdownBg} rounded-md shadow-lg z-20 border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              <button
                onClick={() => handleEdit()}
      ...
      <div className={`${sectionBg} p-3 rounded-md mt-3 shadow-sm`}>
        <div className={`${sectionTextLabel} text-[12px] mb-0.5`}>{t('determinantType')}</div>
      ...
      {/* Work hours */}
      <div className={`${sectionBg} p-3 rounded-md mt-3 shadow-sm`}>
        <div className={`${sectionTextLabel} text-[12px] mb-0.5`}>{t('workHours')}</div>
        <div className={`${sectionTextContent} text-lg`}>{totalHours || 0} {t('hours', { default: "ساعات" })}</div>
      </div>

      {/* Work days */}
      <div className={`${sectionBg} p-3 rounded-md mt-3 shadow-sm`}>
        <div className={`${sectionTextLabel} text-[12px] mb-0.5`}>{t('workdays')}</div>
          ...
          {daysOfWeek.map((day, index) => (
            ...
            >
              {activeDays.includes(day) ? (
                ...
                <div className={`${isDarkMode ? 'bg-gray-500' : 'bg-gray-300'} w-5 h-5 flex items-center justify-center rounded`} />
              )}
        ...
        <div className={`${sectionBg} p-3 rounded-md mt-3 shadow-sm`}>
          ...
          <div className="flex flex-row-reverse gap-3 justify-end">
            {branchNames.map((branch, index) => (
              <div
                key={index}
                className={`${branchTagBg} ${branchTagHover} transition-colors rounded-full px-6 py-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}
              >
                فرع {branch}
              </div>
            ))}
      ...
      {constraint.notes && (
        <div className={`${sectionBg} p-3 rounded-md mt-3 shadow-sm`}>
          <div className={`${sectionTextLabel} text-[12px] mb-0.5`}>{t('notes', { default: "ملاحظات" })}</div>
          <div className={`${sectionTextContent} text-lg`}>{constraint.notes}</div>
        </div>
      )}
    </div>
  );
